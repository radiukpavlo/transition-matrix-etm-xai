...
## Status note
Per the latest user instruction for this sandbox run, **the MNIST pipeline was not executed here**. The repository contains a complete and runnable MNIST implementation, and the MNIST dataset was extracted under `data/mnist/` in torchvision-compatible form. Running the commands in the `README.md` will generate all MNIST artifacts (matrices, figures, logs, and metrics) deterministically.
