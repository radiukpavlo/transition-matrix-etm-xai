...
#### Візуалізація "Хаос проти Порядку" (MNIST)

Для наочного порівняння поведінки методів на обернених даних MNIST ми застосували три підходи зменшення розмірності до векторів помилок реконструкції ($\hat{B} - B_{target}$) для зображень, обернених на різні кути.

##### 1. PCA (Principal Component Analysis)

Лінійний метод, що зберігає основні напрямки варіативності. Кольори відповідають куту обертання.
![MNIST PCA](outputs/mnist/figures/09a_mnist_scatter_pca.png)

##### 2. MDS (Multidimensional Scaling)

Нелінійний метод, що зберігає попарні відстані між векторами помилок.
![MNIST MDS](outputs/mnist/figures/09b_mnist_scatter_mds.png)

##### 3. t-SNE (t-Distributed Stochastic Neighbor Embedding)

Нелінійний метод, оптимізований для виявлення локальної кластерної структури.
...
> **Аналіз:** На всіх чотирьох візуалізаціях видно, що помилки реконструкції для $T_{new}$ демонструють більш впорядковану структуру, ніж для $T_{old}$. Еквіваріантна матриця переходу забезпечує стабільніше "розуміння" геометрії простору даних навіть при значних обертаннях вхідних зображень.
