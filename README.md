...
#### Візуалізація "Хаос проти Порядку" (Синтетика)

Ми візуалізуємо **передбачені латентні вектори** ($B^*$), розфарбовані за **класами** (0, 1, 2...).

* **Ліворуч ($T_{old}$):** При обертанні вхідних даних точки "розлітаються", змішуючи кластери. Це демонструє "хаос" і втрату семантичної структури.
* **Праворуч ($T_{new}$):** Кластерна структура зберігається. Точки одного класу залишаються згрупованими, навіть якщо весь кластер зміщується під дією трансформації.
...
> **Аналіз:** На графіках чітко видно різницю. Для baseline-методу ($T_{old}$) обертання призводить до повного перемішування точок різних класів (overlapping). Для еквіваріантного методу ($T_{new}$) класи залишаються розділеними (separable), що критично важливо для подальших задач класифікації чи інтерпретації. Це і є візуальним доказом "стійкості" (robustness).
...
Ми реконструюємо вектори зображень після обертання та візуалізуємо їх у 2D, розфарбовуючи за **цифрою** (0-9).

* **$T_{old}$ (Baseline):** Точки різних цифер змішуються. Геометричні перетворення руйнують семантичний простір.
* **$T_{new}$ (Equivariant):** Цифри залишаються у своїх кластерах. Еквіваріантність забезпечує, що поворот зображення "трійки" все ще проектується в область "трійок" у латентному просторі (або ж зсувається передбачуваним чином, не перетинаючись з "вісімками").
...
> **Аналіз:** Візуалізації підтверджують гіпотезу на реальних даних. UMAP та t-SNE особливо яскраво показують, як $T_{new}$ зберігає чіткі "острівці" для кожної цифри навіть при значних обертаннях вхідних даних, тоді як $T_{old}$ демонструє розмиті границі та перекриття класів.
